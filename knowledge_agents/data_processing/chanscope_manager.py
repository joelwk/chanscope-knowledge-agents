...
class ChanScopeDataManager:
    ...
    async def generate_embeddings(self, force_refresh: bool = False) -> bool:
        ...
        try:
            ...
            # Uses updated KnowledgeDocument class that handles both content and text_clean fields
    ...
    async def query_relevant_threads(self, query_text: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Verify that at least one text column exists before proceeding
            if "text_clean" not in stratified_sample.columns and "content" not in stratified_sample.columns:
                logger.error("Stratified sample missing both 'text_clean' and 'content' columns - no searchable text available")
                raise ValueError("No text field available for search in stratified sample")
                
            # No need to convert content to text_clean anymore - strings_ranked_by_relatedness 
            # now handles this column mapping automatically with proper detection of either column
